...
Server::Server(int port, const std::string& password)
    ...
    try {
        ...
        std::cerr << "Error: Failed to allocate CommandHandler: " << e.what() << std::endl;
        throw; // Re-throw to indicate critical failure during server initialization
    }
}
...
void Server::start() {
    try {
        ...
        std::cerr << "Error: Socket setup failed: " << e.what() << std::endl;
        return; // Indicate failure to start the server
    }
	_running = true;
